...
\begin{equation}
\% PNR = \frac{W_{sc} - W {pc}}{W_{pc}} \cdot 100 \% = 7,10 \%
\end{equation}
...
\subsection{Características Internas}
Se comprobó que efectivamente la resistencia de campo es mucho más grande que la de armadura y serie. Además se asume que la máquina posee un aislamiento del tipo A,B o E, esto debido a que el efecto de la temperatura en las resistencias depende de dicho aislamiento. La temperatura aumenta enormemente el valor de las resistencias medidas, por lo tanto de no tomarlo en cuenta se tendría un modelo errado. \\
	\begin{itemize}
	\item \textbf{Predeterminar curva de velocidad en función de la tensión de entrada}
\end{itemize}
\begin{figure}[H]
	\centering
...
\subsection{Medición de pérdidas en vacío}
Se realizaron las siguientes mediciones:
\begin{table}[H]
	\centering
...
Para los correctos calculos se requerian de 4 mediciones, debido a que se tomaron 2 no es posible calcular las perdidas en vacío.
